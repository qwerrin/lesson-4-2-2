...
| 課題 | 内容 | 場所 |
|---|---|---|
| 課題1 | 数当てゲーム | このファイル以下（`guess_game.py`） |
| 課題2 | データ処理（CSV ファイルの集計） | [`task2/`](task2/README.md) |
| 課題3 | グラフ作成と画像保存 | [`task3/`](task3/README.md) |

課題1・課題2は標準ライブラリだけで動く。課題3のみ matplotlib が要る。
...
| `verify_binary_search.py` | 追加機能の「7 回制限」に根拠があるかを確かめた検証スクリプト |
...
真ん中を選び続ける戦略なら 6 手で当たる場面だった。

| | 手順 | 手数 |
|---|---|---|
| この記録 | 50 → 25 → **13** → 18 → 20 → 22 → 23 | 7 |
| 真ん中戦略 | 50 → 25 → **12** → 18 → 21 → 23 | 6 |

3 手目で 12 ではなく 13 を選んだ差が最後まで残っている。
7 回という上限は、**真ん中から 1 つずれる程度の余裕しかない**。
...
定数で持つと `MAX_NUMBER` を変えたときに破綻するからだ。範囲を 1〜200 にした時点で
8 手必要になり、7 手制限では絶対に勝てないゲームになってしまう。

関数として切り出しているのは、テストから呼べるようにするため。式を定数の右辺に直接書くと、
テスト側が同じ計算をやり直すだけになり、実装を検証できなくなる。
...
ただし式だけでは足りるかどうか実感できなかったため、1〜100 のすべての答えについて手数を数えた。
